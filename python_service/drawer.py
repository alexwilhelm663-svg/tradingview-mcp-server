...
def main():
    try:
        # Lese Daten aus stdin statt aus Kommandozeilenargumenten (E2BIG-Fix)
    ...
    for w in waves:
        ...
        if w_date in df.index:
            ...
            # Falls Datum leicht abweicht, nimm das nächste verfügbare
            target_date = df.index[df.index.get_indexer([w_date], method='nearest')[0]]
    ...
    fig, axes = mpf.plot(
        ...
        type='candle', 
        style=s, 
        alines=dict(alines=lines, colors=['#00bcd4'], linewidths=1.5),
        returnfig=True,
        figsize=(10, 6)
    )
